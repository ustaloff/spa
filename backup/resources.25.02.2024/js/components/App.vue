...
watch(() => router.currentRoute.value, currentRoute => {
        ...
        //let routeName = route.name || route.matched[0].name;
        //console.log(route.matched[0].name);

        /*let main = route.matched.map((item) => {
            return item.name;
        }).includes('main');*/

        let profile = route.matched.map((item) => {
            return item.name;
        }).includes('profile');

        let dashboard = route.matched.map((item) => {
            return item.name;
        }).includes('dashboard');

        //let layout = 'main';
        //let layout = (dashboard ? 'dashboard');
...
let Top = defineAsyncComponent(() => import('@/components/Top.vue'));
